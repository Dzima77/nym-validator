APP_STATE_ORIGINAL=\"app_hash\": \"\"

# to create new one, just use some escape generator + remember to explicitly include newlines
# for reference, this tool worked for me: https://www.freeformatter.com/javascript-escape.html
define APP_STATE_REPLACEMENT=
\"app_hash\":\"\",\r\n  \"app_state\":{  \r\n    \"systemProperties\":{  \r\n      \"watcherThreshold\":2,\r\n      \"verifierThreshold\":2,\r\n      \"redeemerThreshold\":2,\r\n      \"pipeAccount\":\"0xb749305b3293477b4d6b498b22db5353c9acb3f1\",\r\n      \"coconutProperties\":{  \r\n        \"q\":5,\r\n        \"threshold\":3\r\n      }\r\n    },\r\n    \"accounts\":[  \r\n      {  \r\n        \"address\":\"0x382527A35984E3DC79E239294cdA10138C161686\",\r\n        \"balance\":1000000\r\n      },\r\n      {  \r\n        \"address\":\"0xE0085f37D2227f2ab2F8a49307aA7EEd0fafBc19\",\r\n        \"balance\":42\r\n      }\r\n    ],\r\n    \"issuingAuthorities\":[  \r\n      {  \r\n        \"vk\":\"\",\r\n        \"pub_key\":\"\"\r\n      },\r\n      {  \r\n        \"vk\":\"\",\r\n        \"pub_key\":\"\"\r\n      },\r\n      {  \r\n        \"vk\":\"\",\r\n        \"pub_key\":\"\"\r\n      },\r\n      {  \r\n        \"vk\":\"\",\r\n        \"pub_key\":\"\"\r\n      },\r\n      {  \r\n        \"vk\":\"\",\r\n        \"pub_key\":\"\"\r\n      }\r\n    ],\r\n    \"ethereumWatchers\":[  \r\n      {  \r\n        \"pub_key\":\"BA6P\/+4+FYBZjnaxJkSo38hWdtxcsgOpXeIFmkGIj2iQIXMhitS9O196awJ5CAy8wM7AHBOoKMPYhG4nUGYAOx8=\"\r\n      },\r\n      {  \r\n        \"pub_key\":\"BNNqH8evAM\/w97DYVWhS4g9W3Qf68l5nGCCqXopTq8iYV+hGd4ISwPm8z4uomRkWD9VLySt3ggBvlqQLSSJw0Lc=\"\r\n      },\r\n      {\r\n        \"pub_key\":\"BFFBP6SdRojxZhpejoOGoQnGFzDOlmStsdjBuLXeFeHau7HBPmnBDwZyBZUwKrCR3BrqAKJnv3PfbTBJsDEqe9Y=\"\r\n      },\r\n      {\r\n        \"pub_key\":\"BCvLQP7AM5kcovDJEiMzoQ89xHHrYG4ASPIT9NH7Sys2sZu4y3Vc\/d3yP8wC5l769BtXTXeIzRLgSubTCDBq9\/M=\"\r\n      }\r\n    ],\r\n    \"verifiers\":[  \r\n      {  \r\n        \"pub_key\":\"BH3yFGQi\/qSbqolZHLy0bObXEUHNrC6Y++Wz7yQUPOJU5yooal6ZGP5sM7WHILt6cA39o5879r4FHzuiw4ydz5g=\"\r\n      },\r\n      {  \r\n        \"pub_key\":\"BANigUwYucmc9ZA7UCH9zttrApF75jV87vzMNEk2ewhsDTsjdXZhFkjGE8qkA7rs7WaU4VtXmxJBqCsGNa7\/Nec=\"\r\n      }\r\n    ],\r\n    \"redeemers\":[\r\n      {\r\n        \"pub_key\":\"BHUKLaSo9qAnli08HmDqNaiM3HkKiwRa6UoD6pI+xgHEozrheUSs7p0e6CphKJnJMTgUW7W+dp4Gs6rl21rTKp8=\"\r\n      },\r\n      {\r\n        \"pub_key\":\"BOtjnyCjFT8LjI2i41n0qW5h51oLSEOR6fjJeMfrmnsqEsVycV+I+fFBzX8GsJztU+Z1v4FsdD8dZo+bAKLF68A=\"\r\n      },\r\n      {\r\n        \"pub_key\":\"BEmDqAFglt3A2LeMKtxXIt11vK2SAm4Z72LJ6vR1nF7Aqiuf8Y65LP2AkFErDHtNl5KdYEBABe36KodVQEpCBM0=\"\r\n      }\r\n    ]\r\n  }
endef

# TODO: just make each 'entity' have 1 of each: watcher, validator, node
# modifying this also requires updating docker-compose
NUM_WATCHERS=4
NUM_VERIFIERS=2
NUM_VALIDATORS=3
NUM_NODES=4
NUM_PROVIDERS=2
NUM_REDEEMERS=3
# requires presence of appropriate keys
THRESHOLD=3

all:
	make localnet-build

build_socket_client:
	dep ensure
	go build -o $(GOPATH)/bin/nym-socket-client ./cmd/nym-socket-client

build_local_validators:
	@if ! [ -f build/validators/validator1/config.toml ]; then \
		i=1; while [ "$$i" -le $(NUM_VALIDATORS) ]; do \
			mkdir -p build/validators/validator$$i/coconutkeys ;\
			cp localnetdata/validators/configs/config$$i.toml build/validators/validator$$i/config.toml ;\
			cp localnetdata/validators/keys/coconutkeys/threshold-secretKey-id=$$i-attrs=5-n=5-t=$(THRESHOLD).pem build/validators/validator$$i/coconutkeys/ ;\
			cp localnetdata/validators/keys/coconutkeys/threshold-verificationKey-id=$$i-attrs=5-n=5-t=$(THRESHOLD).pem build/validators/validator$$i/coconutkeys/ ;\
			i=$$((i + 1));\
		done ;\
	fi
	docker build -t nym/nym-validator -f ./DOCKER/validator/Dockerfile .

build_local_providers:
	@if ! [ -f build/providers/provider1/config.toml ]; then \
		i=1; while [ "$$i" -le $(NUM_PROVIDERS) ]; do \
			mkdir -p build/providers/provider$$i/accountKey ;\
			mkdir -p build/providers/provider$$i/validatorKeys ;\
			cp localnetdata/providers/configs/config$$i.toml build/providers/provider$$i/config.toml ;\
			cp localnetdata/providers/keys/accountKeys/provider$$i.key build/providers/provider$$i/accountKey/provider.key ;\
			cp localnetdata/validators/keys/coconutkeys/threshold-verificationKey-* build/providers/provider$$i/validatorKeys/ ;\
			i=$$((i + 1));\
		done ;\
	fi
	docker build -t nym/nym-provider -f ./DOCKER/provider/Dockerfile .


build_local_nym_tendermint_nodes:
	@if ! [ -f build/nodes/node0/config/genesis.json ]; then  \
		i=0; while [ "$$i" -lt $(NUM_NODES) ]; do \
			mkdir -p build/nodes/node$$i/config; \
			i=$$((i + 1));\
		done ;\
		chmod g+w -R build/nodes; \
		docker run --user $$(id -u $${USER}):$$(id -g $${USER}) --rm -v $(CURDIR)/build/nodes:/tendermint:Z tendermint/tendermint testnet --v $(NUM_NODES) --o . --populate-persistent-peers --starting-ip-address 192.167.10.2 ; \
	fi
	i=0; while [ "$$i" -lt $(NUM_NODES) ]; do \
		sed -i -e "s/$(APP_STATE_ORIGINAL)/$(APP_STATE_REPLACEMENT)/g" build/nodes/node$$i/config/genesis.json ; \
		i=$$((i + 1));\
	done ;\
	docker build -t nym/nym-tendermint-node -f ./DOCKER/nym-tendermint-node/Dockerfile .

build_local_ethereum_watchers:
	@if ! [ -f build/eth_watchers/watcher1/config.toml ]; then \
		i=1; while [ "$$i" -le $(NUM_WATCHERS) ]; do \
			mkdir -p build/ethereum-watchers/watcher$$i; \
			cp localnetdata/ethereum-watchers/configs/config$$i.toml build/ethereum-watchers/watcher$$i/config.toml ;\
			cp localnetdata/ethereum-watchers/keys/watcher$$i.key build/ethereum-watchers/watcher$$i/watcher.key ;\
			i=$$((i + 1));\
		done ;\
	fi
	docker build -t nym/nym-ethereum-watcher -f ./DOCKER/ethereum-watcher/Dockerfile .

build_local_verifiers:
	@if ! [ -f build/verifiers/verifier1/config.toml ]; then \
		i=1; while [ "$$i" -le $(NUM_VERIFIERS) ]; do \
			mkdir -p build/verifiers/verifier$$i/validatorKeys ;\
			cp localnetdata/verifiers/configs/config$$i.toml build/verifiers/verifier$$i/config.toml ;\
			cp localnetdata/verifiers/keys/verifier$$i.key build/verifiers/verifier$$i/verifier.key ;\
			cp localnetdata/validators/keys/coconutkeys/threshold-verificationKey-* build/verifiers/verifier$$i/validatorKeys/ ;\
			i=$$((i + 1));\
		done ;\
	fi
	docker build -t nym/nym-verifier -f ./DOCKER/verifier/Dockerfile .


build_local_redeemers:
	@if ! [ -f build/redeemers/redeemer1/config.toml ]; then \
		i=1; while [ "$$i" -le $(NUM_REDEEMERS) ]; do \
			mkdir -p build/redeemers/redeemer$$i ;\
			cp localnetdata/redeemers/configs/config$$i.toml build/redeemers/redeemer$$i/config.toml ;\
			cp localnetdata/redeemers/keys/redeemer$$i.key build/redeemers/redeemer$$i/redeemer.key ;\
			cp localnetdata/redeemers/keys/pipeaccount.key build/redeemers/redeemer$$i/pipeAccount.key ;\
			i=$$((i + 1));\
		done ;\
	fi
	docker build -t nym/nym-redeemer -f ./DOCKER/redeemer/Dockerfile .

build_local_faucet:
	@if ! [ -f build/faucet/config.toml ]; then \
		mkdir -p build/faucet ;\
		cp localnetdata/faucet/config.toml build/faucet/config.toml ;\
		cp localnetdata/faucet/faucet.key build/faucet/faucet.key ;\
	fi
	docker build -t nym/nym-faucet -f ./DOCKER/faucet/Dockerfile .

build_dockerfiles:
	dep ensure
	docker build -t nym/nym-validator -f ./DOCKER/validator/Dockerfile .
	docker build -t nym/nym-provider -f ./DOCKER/provider/Dockerfile .
	docker build -t nym/nym-tendermint-node -f ./DOCKER/nym-tendermint-node/Dockerfile .
	docker build -t nym/nym-ethereum-watcher -f ./DOCKER/ethereum-watcher/Dockerfile .
	docker build -t nym/nym-verifier -f ./DOCKER/verifier/Dockerfile .
	docker build -t nym/nym-redeemer -f ./DOCKER/redeemer/Dockerfile .
	docker build -t nym/nym-faucet -f ./DOCKER/faucet/Dockerfile .

build_binaries:
	dep ensure
	go build -o $(GOPATH)/bin/nym-validator ./cmd/nym-validator
	go build -o $(GOPATH)/bin/nym-provider ./cmd/nym-provider
	go build -o $(GOPATH)/bin/nym-tendermint-node ./cmd/nym-tendermint-node
	go build -o $(GOPATH)/bin/nym-ethereum-watcher ./cmd/nym-ethereum-watcher
	go build -o $(GOPATH)/bin/nym-verifier ./cmd/nym-verifier
	go build -o $(GOPATH)/bin/nym-redeemer ./cmd/nym-redeemer-demo
	go build -o $(GOPATH)/bin/nym-faucet ./cmd/nym-faucet-demo

localnet-build:
	dep ensure
	make build_local_nym_tendermint_nodes
	make build_local_validators
	make build_local_ethereum_watchers
	make build_local_providers
	make build_local_verifiers
	make build_local_redeemers
	make build_local_faucet

build_testnet_binaries:
	dep ensure
	go build -o build/testnet-binaries/nym-validator ./cmd/nym-validator
	go build -o build/testnet-binaries/nym-tendermint-node ./cmd/nym-tendermint-node
	go build -o build/testnet-binaries/nym-ethereum-watcher ./cmd/nym-ethereum-watcher
	go build -o build/testnet-binaries/nym-verifier ./cmd/nym-verifier
	go build -o build/testnet-binaries/nym-redeemer ./cmd/nym-redeemer-demo


# Run a local testnet consisting currently of:
# 4 tendermint nodes
# 3 issuing authorities
# 2 credential verifiers
# 2 ethereum watchers
# 3 token redeemers
# 2 service providers to choose from
localnet-start:
	@if ! [ -f build/nodes/node0/config/genesis.json ]; then make localnet-build; fi
	docker-compose up

# Stop testnet
localnet-stop:
	docker-compose down

localnet-clear:
	make localnet-stop
	rm -rf build