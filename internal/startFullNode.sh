...
if (( $1 > 0 && $1 <= NUM_NODES )); then 
    ...
    tmux send-keys -t $session:0 "nym-tendermint-node run --cfgFile ~/nymnet/node/config/config.toml --dataRoot ~/nymnet/node/" 
    ...
    tmux send-keys -t $session:1 "nym-validator run --f ~/nymnet/validator/config.toml" 
    tmux send-keys -t $session:1 C-m 

    tmux new-window -t $session:2 -n "ethw."
    tmux select-window -t $session:2
    tmux send-keys -t $session:2 "nym-ethereum-watcher run --f ~/nymnet/ethereum-watcher/config.toml" 
    tmux send-keys -t $session:2 C-m 

    tmux new-window -t $session:3 -n "ver."
    tmux select-window -t $session:3
    tmux send-keys -t $session:3 "nym-verifier run --f ~/nymnet/verifier/config.toml" 
    tmux send-keys -t $session:3 C-m 

    tmux new-window -t $session:4 -n "red."
    tmux select-window -t $session:4
    tmux send-keys -t $session:4 "nym-redeemer run --f ~/nymnet/redeemer/config.toml" 
    tmux send-keys -t $session:4 C-m 

    if (($1 == 1)); then 
        tmux new-window -t $session:5 -n "fauc."
        tmux select-window -t $session:5
        tmux send-keys -t $session:5 "nym-faucet run --f ~/nymnet/faucet/config.toml" 
    ...
    if (($1 == 2 || $1 == 3)); then
        ...
        tmux select-window -t $session:5
        tmux send-keys -t $session:5 "nym-provider run --f ~/nymnet/provider/config.toml" 
        tmux send-keys -t $session:5 C-m 
    fi
