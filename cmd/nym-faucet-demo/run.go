// run.go - faucet startup definition.
...
import (
	...
	"github.com/nymtech/nym-validator/faucet"
...
func cmdRun(args []string, usage string) {
	...
	daemon.Start(func(args []string) daemon.Service {
		cfgFile := opts.Flags("--f").Label("CFGFILE").String("Path to the config file of the faucet", defaultConfigFile)
		...
		// Start up the faucet.
		faucet, err := faucet.New(cfg)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Failed to spawn faucet instance: %v\n", err)
			os.Exit(-1)
		}
